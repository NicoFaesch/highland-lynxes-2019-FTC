...
public class FTC_2019_Tele extends OpMode
{
    ...
    public void loop() {
            ...
            //resets the robot if flipped
            if(gamepad1.dpad_down == true) 
            {
                if((Math.round(pot_arm.getVoltage() / 0.033 * 3.6 * 10d) / 10d) > 80) {
                    arm_down = false;
                    motor_arm_left.setTargetPosition(-2000);
                    motor_arm_right.setTargetPosition(-2000);
            ...
            //reset arm using potentiometer
            
            if(gamepad2.x == true) {
                if((Math.round(pot_arm.getVoltage() / 0.033 * 3.6 * 10d) / 10d) > 160) {
                    arm_down = false;
            ...
            telemetry.addData("Encoder",Integer.valueOf(motor_arm_left.getCurrentPosition()).toString());
